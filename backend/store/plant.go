...
func FetchPlantBySensorID(ctx context.Context, sensorID string) (*models.Plant, error) {
	...
	err := db.QueryRowContext(ctx, `SELECT id, sensor_id, name, date_planted, created_at, updated_at FROM plants WHERE sensor_id = $1 ORDER BY id DESC LIMIT 1`, sensorID).
